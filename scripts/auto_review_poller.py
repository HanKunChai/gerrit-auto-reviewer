...
class AutoReviewPoller:
    ...
    def _review_change(
        ...
        # 3. Claude Code AI 评审 (在 repo 目录中运行，可读取完整文件)
        ...
        if claude_review:
            ...
            # Claude 失败 → 不向 Gerrit 上报任何信息，只记录日志
            logger.error("  Claude Code 评审失败，跳过上报 (变更: %s)", change_id)
            return

        # 4. 构造评审消息
        logger.info("  [4/4] 构造评审意见...")
        message = self._build_review_message(change_id, subject, issues, claude_review)
    ...
    def _invoke_claude(
        ...
        Passes commit SHA so the prompt uses a stable ref.
        """
        ...
        # Build prompt — 传文件名+SHA，Claude 自己取 diff
        ...
        timeout = self.config.auto_review.claude_timeout
        logger.info("  调用 Claude Code (cwd=%s, prompt=%d chars, timeout=%ds)...",
                    repo_dir, len(prompt), timeout)
    ...
    def _build_review_message(
        ...
        # Claude Code AI 评审 (主)
        if claude_review:
            # 归一化：如果已有 [已解决/部分解决/未解决] 结论，去掉末尾冗余的 No issues found.
            normalized = re.sub(
                r"\n?\s*No issues found\.?\s*$", "",
